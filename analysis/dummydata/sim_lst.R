...
sim_list_vax <- lst(
  ...
  covid_vax_disease_2_day = bn_node(
    ~as.integer(runif(n=..n, covid_vax_disease_1_day+15, covid_vax_disease_1_day+100)),
    needs = c("covid_vax_disease_1_day"),
    missing_rate = ~0
  ),
  covid_vax_disease_3_day = bn_node(
    ~as.integer(runif(n=..n, covid_vax_disease_2_day + 150, covid_vax_disease_2_day + 210)),
    needs = c("covid_vax_disease_2_day"),
    missing_rate = ~0.1
  ),
  covid_vax_disease_4_day = bn_node(
    ~as.integer(runif(n=..n, covid_vax_disease_3_day+200, covid_vax_disease_3_day+440)),
    needs = c("covid_vax_disease_3_day"),
    missing_rate = ~0.5
  ),
  covid_vax_disease_5_day = bn_node(
    ~as.integer(runif(n=..n, covid_vax_disease_4_day+50, covid_vax_disease_4_day+100)),
    needs = c("covid_vax_disease_4_day"),
    missing_rate = ~0.8
  ),
  covid_vax_disease_6_day = bn_node(
    ~as.integer(runif(n=..n, covid_vax_disease_5_day+50, covid_vax_disease_5_day+100)),
    needs = c("covid_vax_disease_5_day"),
    missing_rate = ~0.9
  ),
...
sim_list_demographic <- lst(
  ...
  ethnicity = bn_node(
    ~rfactor(
      n=..n, 
      levels = c("White", "Mixed", "Asian or Asian British", "Black or Black British", "Other", "Unknown"), 
      p = c(0.75, 0.05, 0.05, 0.05, 0.05, 0.05)
      ),
    missing_rate = ~ 0
  ),
